  ...
  <div class="app-container">
    ...
    <div id="flowWrap" ref="flowWrap" class="flow-wrapper">
      <div id="table-flow">
        ...
        <TableNode v-for="node in json.nodes" :id="node.name" :key="node.name" :node="node" />
      </div>
    </div>
  </div>
</template>
    ...
    initialize() {
      jsplumb.ready(() => {
        ...
        this.jsplumbInstance.bind('click', (conn) => {
          ...
          let notData = this.notFindActiveNode(this.json.edges, to[0], to[1]);
          ...
          notData.forEach(an => {
            ...
            $("#" + an.tbName + "-cols").find("#" + full).css("background-color", 'black')
          })
