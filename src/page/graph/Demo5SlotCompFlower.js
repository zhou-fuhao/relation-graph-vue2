...
export class Flower {
  ...
  create() {
    ...
    for (let i = rings; i > 0; i--) {
      ...
      this.createPetalsRing(
        layer,
        x,
        y,
        np,
        iRadius + i * iRadiusCoef,
        oRadius + i * oRadiusCoef,
        angle + ((i % 2) * PI) / np,
        color
      );
...
class Petal {
  constructor({
    ...
    this.sp = [
      x + cos(startA + da / 2) * iRadius0,
      y + sin(startA + da / 2) * iRadius0
    ];
    this.cp1 = [
      x + cos(startA - cpda) * (iRadius + cpdr),
      y + sin(startA - cpda) * (iRadius + cpdr)
    ];
    this.cp2 = [
      x + cos(endA + cpda) * (iRadius + cpdr),
      y + sin(endA + cpda) * (iRadius + cpdr)
    ];
    this.ep = [
      x + cos(startA + da / 2) * oRadius,
      y + sin(startA + da / 2) * oRadius
    ];
  ...
  pathD() {
    return [this.curve1.pathD(), this.curve2.pathD()].join(" ");
  }
}
