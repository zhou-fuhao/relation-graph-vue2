...
const comm = {
    ...
    findDownstreamNode(edges, tbName, column) {
        ...
        edges.forEach(edge => {
            if (edge.from.name === tbName && edge.from.field === column) {
                downstreamNodes = downstreamNodes.concat(this.findDownstreamNode(this.json.edges, edge.to.name, edge.to.field))
            }
        })
    ...
    findUpstreamNode(edges, tbName, column) {
        ...
        edges.forEach(edge => {
            if (edge.to.name === tbName && edge.to.field === column) {
                upstreamNodes = upstreamNodes.concat(this.findUpstreamNode(this.json.edges, edge.from.name, edge.from.field))
            }
        })
    ...
    //鼠标移动到连线上时候
    notFindActiveNode(edges, tbName, column) {
        let up = this.notFindUpstreamNode(edges, tbName, column)
        //up数组首个元素重复 需要跳过
        return up.slice(1).reverse()
            .concat(this.notFindDownstreamNode(edges, tbName, column))
    },
    //找下游的子节点
    notFindDownstreamNode(edges, tbName, column) {
        let downstreamNodes = [{
            tbName,
            column
        }]
    ...
    //找上游的父节点
    notFindUpstreamNode(edges, tbName, column) {
        let upstreamNodes = [{
            tbName,
            column
        }]
